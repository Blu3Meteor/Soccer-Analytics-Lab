...
def to_single_playing_direction(home, away, events):
    ...
    '''
    # 1. Flip Tracking Data
    for team in [home, away]:
        # Find the first index/frame where Period is 2
        second_half_idx = (team['Period'] == 2).idxmax()
        columns = [c for c in team.columns if c[-1].lower() in ['x', 'y']]
        team.loc[second_half_idx:, columns] *= -1
        
    # 2. Flip Event Data (Handled separately due to different column names and indices)
    # Find all rows belonging to the second half
    second_half_events = events['Period'] == 2
    event_columns = [c for c in events.columns if c.split()[-1].lower() in ['x', 'y']]
    events.loc[second_half_events, event_columns] *= -1
...
def find_goalkeeper(team):
    ...
    ''' 
    # find first period
    first_half = team[team['Period']==1]
    # distance of each player to home goal line (x=0) or away goal line (x=1) at kick off
    # ... (rest of old code using idxmin(1) or idxmax(1))
