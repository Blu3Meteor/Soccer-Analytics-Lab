...
def calc_player_velocities(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12):
    ...
    second_half_idx = (team['Period'] == 2).idxmax()
    ...
    for player in player_ids: # cycle through players individually
        ...
        if smoothing:
            if filter_=='Savitzky-Golay':
                # Mask out rows where data is missing entirely so we don't pollute the filter
                # Fill remaining NaNs with 0 temporarily for the SciPy filter, then restore NaNs later
                
                # --- First Half ---
                v1_x = vx.loc[:second_half_idx].fillna(0)
                v1_y = vy.loc[:second_half_idx].fillna(0)
                
                vx.loc[:second_half_idx] = signal.savgol_filter(v1_x, window_length=window, polyorder=polyorder, axis=0)
                vy.loc[:second_half_idx] = signal.savgol_filter(v1_y, window_length=window, polyorder=polyorder, axis=0)
                
                # --- Second Half ---
                v2_x = vx.loc[second_half_idx:].fillna(0)
                v2_y = vy.loc[second_half_idx:].fillna(0)
                
                vx.loc[second_half_idx:] = signal.savgol_filter(v2_x, window_length=window, polyorder=polyorder, axis=0)
                vy.loc[second_half_idx:] = signal.savgol_filter(v2_y, window_length=window, polyorder=polyorder, axis=0)
                
                # Put NaNs back where the player originally had no tracking data
                vx[team[player + '_x'].isna()] = np.nan
                vy[team[player + '_y'].isna()] = np.nan
